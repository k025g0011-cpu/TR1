...
inline BuildingCost GetBuildingCost(CellType type) {	//inlineは処理を呼び出し元に展開して高速化。
	...
	case CellType::COMMERCIAL:
		return {500.0f, 4.0f}; // 商業: 建設$500、維持費$20（店舗維持）
	case CellType::INDUSTRIAL:
		return {400.0f, 2.0f}; // 工業: 建設$400、維持費$15（操業コスト）
	case CellType::PARK:
...
private:
	...
	static constexpr float AGE_DECAY = 0.1f;   // 劣化速度：猶予を過ぎた後、1ターンにつき収入効率が2%ずつ低下
